...
describe('ResponseProcessor', () => {
  ...
  describe.skip('circuit breaker: consecutiveSummaryFailures counter (#1633 — deleted)', () => {
    ...
    it('increments the counter when a summary was expected but none was stored', async () => {
      ...
      // XML-shaped response that bypasses the non-XML early-return path but produces no summary.
      // A bare ghost observation (no title/narrative/facts/concepts) is silently dropped by the
      // parser, leaving observations=[] and summary=null, which triggers the circuit-breaker.
