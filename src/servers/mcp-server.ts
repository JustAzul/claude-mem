...
function checkMarketplaceMarker(): void {
  try {
    ...
    const marketplaceCandidates = [
      resolve(home, '.claude', 'plugins', 'marketplaces', 'JustAzul'),
      resolve(home, '.config', 'claude', 'plugins', 'marketplaces', 'JustAzul'),
    ];
    const present = marketplaceCandidates.some(p => p && existsSync(p));
    const cacheCandidates = [
      resolve(home, '.claude', 'plugins', 'cache', 'JustAzul', 'claude-mem'),
      resolve(home, '.config', 'claude', 'plugins', 'cache', 'JustAzul', 'claude-mem'),
    ];
