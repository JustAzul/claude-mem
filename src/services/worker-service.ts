...
import { CustomOpenAIAgent, isCustomOpenAISelected, isCustomOpenAIAvailable } from './worker/CustomOpenAIAgent.js';
...
export class WorkerService implements WorkerRef {
  ...
  constructor() {
    ...
    this.customOpenAIAgent = new CustomOpenAIAgent(this.dbManager, this.sessionManager);
    ...
    this.server = new Server({
      ...
      getAiStatus: () => {
        let provider = 'claude';
        if (isCustomOpenAISelected() && isCustomOpenAIAvailable()) provider = 'custom';
  ...
  private registerRoutes(): void {
    ...
    const sessionRoutes = new SessionRoutes(this.sessionManager, this.dbManager, this.sdkAgent, this.geminiAgent, this.openRouterAgent, this.customOpenAIAgent, this.sessionEventBroadcaster, this, this.completionHandler);
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      logger.debug('WORKER', `file-context token backfill updated=${fileTokenBackfill.updatedCount}`);
      const originBackfill = this.dbManager.getSessionStore().backfillRecentObservationOrigins({
        limit: 200,
        windowDays: MEMORY_ASSIST_RECENT_HYDRATION_WINDOW_DAYS,
      });
      logger.debug('WORKER', `observation origin backfill resolved=${originBackfill.resolvedCount} unresolved=${originBackfill.unresolvedCount}`);
  ...
  private reclassifyAtDispatch(
    ...
    try {
      ...
      // CustomOpenAIAgent has no dedicated classifier yet; fall through to null.
    } catch {
