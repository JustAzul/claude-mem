...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private handleObservationsByClaudeId = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (typeof cwd === 'string' && isProjectExcluded(cwd, settings.CLAUDE_MEM_EXCLUDED_PROJECTS)) {
    ...
    const pendingMessageId = await this.sessionManager.queueObservation(sessionDbId, {
      tool_name,
