...
export class SessionManager {
  ...
  private getQueueEngine(): InspectableObservationQueueEngine {
    if (!this.queueEngine) {
      ...
      if (this.queueEngineName === 'bullmq') {
        ...
        this.queueEngine = new SqliteObservationQueueEngine(sessionStore.db);
      }
    }
  ...
  /**
   * Queue an observation. Auto-initializes session from DB if not in memory.
   * Returns the persisted message id (0 if dedup-suppressed).
   */
  async queueObservation(sessionDbId: number, data: ObservationData): Promise<number> {
    let session = this.sessions.get(sessionDbId);
    if (!session) {
    ...
    try {
      const queue = this.getQueueEngine();
      messageId = await queue.enqueue(sessionDbId, session.contentSessionId, message);
  ...
  /**
   * Drain additional pending observations for batch prompt construction.
   * Returns up to maxItems observation messages that are immediately available.
   * BullMQ engine returns [] (batching only on the SQLite path).
   */
  async tryClaimAdditionalObservations(sessionDbId: number, maxItems: number): Promise<PendingMessageWithId[]> {
    if (maxItems <= 0) return [];
    return await this.getQueueEngine().tryClaimObservationBatch(sessionDbId, maxItems);
  }
