...
export class ClaudeProvider {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    // Trackers feed processAgentResponse with ground-truth tool/capture metadata for the
    // most recently yielded observation. Mutable-object pattern mirrors cwdTracker so the
    // generator writes and the response handler reads without coupling.
  ...
  private async *createMessageGenerator(
    ...
    // Mark init/continuation turns as discovery so ResponseProcessor's type override fires
    // even with no preceding tool call — otherwise LLM can infer 'refactor'/'bugfix' from
    // code it merely read.
    ...
    for await (const message of this.sessionManager.getMessageIterator(session.sessionDbId)) {
      ...
      if (message.type === 'observation') {
        ...
        const additional = await this.sessionManager.tryClaimAdditionalObservations(
          session.sessionDbId,
        ...
        // Clear trackers before yielding so a stale observation context cannot bleed into
        // processAgentResponse if the LLM hallucinates an <observation> inside a summary.
