...
function gitCapture(cwd: string, args: string[]): string | null {
  ...
  if (duration > 1000) {
    logger.debug('PROCESS', `Slow git operation: git -C ${cwd} ${args.join(' ')} took ${duration}ms`);
  }

  if (r.error) {
    logger.warn('PROCESS', `Git operation failed: git -C ${cwd} ${args.join(' ')}`, {
  ...
  if (r.status !== 0) {
    logger.debug('PROCESS', `Git returned non-zero exit code ${r.status}: git -C ${cwd} ${args.join(' ')}`, {
