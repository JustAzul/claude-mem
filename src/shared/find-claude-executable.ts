...
export function findClaudeExecutable(logComponent: string = 'SDK'): string {
  ...
  if (settings.CLAUDE_CODE_PATH) {
    ...
    logger.debug('SDK_SPAWN', `Using configured CLAUDE_CODE_PATH: ${settings.CLAUDE_CODE_PATH} (${version})`);
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (version) {
        logger.debug('SDK_SPAWN', `Using claude.cmd from PATH (${version})`);
        return 'claude.cmd';
      }
      logger.warn('SDK_SPAWN', 'claude.cmd found in PATH but failed --version check, trying next candidate');
    } catch {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (version) {
        logger.debug('SDK_SPAWN', `Auto-detected Claude CLI: ${candidate} (${version})`);
        return candidate;
      }
    ...
    if (error instanceof Error) {
      logger.debug('SDK_SPAWN', 'Claude executable auto-detection failed', {}, error);
    } else {
      logger.debug('SDK_SPAWN', 'Claude executable auto-detection failed with non-Error', {}, new Error(String(error)));
    }
  }
