"use strict";var Qs=Object.create;var le=Object.defineProperty;var Zs=Object.getOwnPropertyDescriptor;var en=Object.getOwnPropertyNames;var tn=Object.getPrototypeOf,sn=Object.prototype.hasOwnProperty;var nn=(t,e)=>{for(var s in e)le(t,s,{get:e[s],enumerable:!0})},ot=(t,e,s,n)=>{if(e&&typeof e=="object"||typeof e=="function")for(let r of en(e))!sn.call(t,r)&&r!==s&&le(t,r,{get:()=>e[r],enumerable:!(n=Zs(e,r))||n.enumerable});return t};var H=(t,e,s)=>(s=t!=null?Qs(tn(t)):{},ot(e||!t||!t.__esModule?le(s,"default",{value:t,enumerable:!0}):s,t)),rn=t=>ot(le({},"__esModule",{value:!0}),t);var Yn={};nn(Yn,{generateContext:()=>nt});module.exports=rn(Yn);var qs=H(require("path"),1),Ks=require("os"),Js=require("fs");var Qt=require("bun:sqlite");var N=require("path"),$e=require("os"),ue=require("fs");var ct=require("url");var $=require("fs"),z=require("path"),at=require("os"),Ue=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(Ue||{}),it=(0,z.join)((0,at.homedir)(),".claude-mem"),ke=class{level=null;useColor;logFilePath=null;logFileInitialized=!1;constructor(){this.useColor=process.stdout.isTTY??!1}ensureLogFileInitialized(){if(!this.logFileInitialized){this.logFileInitialized=!0;try{let e=(0,z.join)(it,"logs");(0,$.existsSync)(e)||(0,$.mkdirSync)(e,{recursive:!0});let s=new Date().toISOString().split("T")[0];this.logFilePath=(0,z.join)(e,`claude-mem-${s}.log`)}catch(e){console.error("[LOGGER] Failed to initialize log file:",e),this.logFilePath=null}}}getLevel(){if(this.level===null)try{let e=(0,z.join)(it,"settings.json");if((0,$.existsSync)(e)){let s=(0,$.readFileSync)(e,"utf-8"),r=(JSON.parse(s).CLAUDE_MEM_LOG_LEVEL||"INFO").toUpperCase();this.level=Ue[r]??1}else this.level=1}catch{this.level=1}return this.level}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.getLevel()===0?`${e.message}
...
`);return mn(o)}function q(t,e){if(!t)return e;try{return JSON.parse(t)}catch{return e}}function G(t){return t==null?null:JSON.stringify(t)}function ft(t){t.run(`
  ...
  `).run(e.source,e.status,e.reason,e.project??null,e.platformSource??null,e.sessionDbId??null,e.contentSessionId??null,e.promptNumber??null,e.threshold??null,e.bestDistance??null,e.worstDistance??null,e.candidateCount??null,e.selectedCount??null,e.promptLength??null,e.filePath??null,e.message??null,e.estimatedInjectedTokens??null,G(e.traceItems??[]),G(e.selectedIds??null),G(e.shadowRanking??null),e.systemVerdict??null,e.systemConfidence??null,G(e.systemReasons??[]),G(e.systemEvidence??null),e.userFeedback??null,s,s),o=pn(t,Number(r.lastInsertRowid));return l.debug(`[memory-assist-decisions] stored decision ${o.id??"unknown"}`),o}function pn(t,e){let s=t.prepare(`
    ...
    WHERE id = ?
  `).get(e);return s?Se(s):null}function Q(t,e={}){let s=Math.min(Math.max(e.limit??20,1),1e4),n=[],r=[];e.windowDays&&(n.push("created_at_epoch >= ?"),r.push(Date.now()-e.windowDays*24*60*60*1e3)),e.source&&(n.push("source = ?"),r.push(e.source)),e.project&&(n.push("project = ?"),r.push(e.project)),e.contentSessionId&&(n.push("content_session_id = ?"),r.push(e.contentSessionId));let o=n.length>0?`WHERE ${n.join(" AND ")}`:"",a=t.prepare(`
    SELECT *
    FROM memory_assist_decisions
    ${o}
    ORDER BY created_at_epoch DESC
    LIMIT ${s}
  `).all(...r).map(Se);return l.debug(`[memory-assist-decisions] loaded ${a.length} recent decisions (limit=${s})`),a}function St(t,e,s,n,r=Date.now()){let i=r-n,a=r+100;return t.prepare(`
    SELECT *
    FROM memory_assist_decisions
  ...
  `).all(e,s,i,a).map(Se)}function yt(t,e,s,n,r,o){t.prepare(`
    ...
    SET system_verdict = ?,
        ...
        updated_at_epoch = ?
    WHERE id = ?
  `).run(s,n,G(r),G(o),Date.now(),e)}function Ot(t,e,s){t.prepare(`
    UPDATE memory_assist_decisions
    SET user_feedback = ?,
        updated_at_epoch = ?
    WHERE id = ?
  `).run(s,Date.now(),e)}function Rt(t,e,s,n){let r=s-n,o=t.prepare(`
    ...
    WHERE content_session_id = ?
      ...
      AND prompt_number >= ?
      AND prompt_number < ?
  `).all(e,r,s),i=new Set;for(let a of o){let c=q(a.selected_ids_json,[]);for(let d of c)Number.isFinite(d)&&d>0&&i.add(d)}return i}function Xe(t,e){if(e.length===0)return[];let s=e.map(()=>"?").join(", ");return t.prepare(`
    SELECT *
    FROM memory_assist_decisions
    WHERE id IN (${s})
  `).all(...e).map(Se)}function Z(t,e){if(!t)return e;try{return JSON.parse(t)}catch{return e}}function ee(t){return t==null?null:JSON.stringify(t)}function At(t){t.run(`
  ...
  `).run(e.decisionId??null,e.pendingMessageId??null,e.source??null,e.promptNumber??null,e.contentSessionId??null,e.sessionDbId??null,e.project??null,e.platformSource??null,e.signalType,e.toolName,e.action,e.filePath??null,ee(e.relatedFilePaths??[]),ee(e.concepts??[]),ee(e.generatedObservationIds??[]),ee(e.metadata??{}),s),r=t.prepare(`
    SELECT *
    FROM memory_assist_outcome_signals
    WHERE id = ?
  `).get(Number(n.lastInsertRowid)),o=r?Nt(r):{...e,id:Number(n.lastInsertRowid),timestamp:s};return l.debug(`[memory-assist-outcomes] stored outcome signal ${o.id??"unknown"}`),o}function vt(t,e,s){return s.length===0?[]:t.transaction(()=>{let n=t.prepare(`
      SELECT id, generated_observation_ids_json
      FROM memory_assist_outcome_signals
      WHERE pending_message_id = ?
      ORDER BY id DESC
      LIMIT 1
    `).get(e);if(!n)return l.debug(`[memory-assist-outcomes] no outcome signal found for pending message ${e}`),[];let r=Z(n.generated_observation_ids_json,[]),o=[...new Set([...r,...s])];return t.prepare(`
      UPDATE memory_assist_outcome_signals
      SET generated_observation_ids_json = ?
      WHERE id = ?
    `).run(ee(o),n.id),l.debug(`[memory-assist-outcomes] attached ${s.length} observations to pending message ${e}`),o})()}function Ct(t,e){if(e.length===0)return{};let s=e.map(()=>"?").join(", "),n=t.prepare(`
    SELECT *
    FROM memory_assist_outcome_signals
  ...
  `).get(e).cnt,a=t.prepare(`
    ...
    WHERE d.status = 'injected' AND d.created_at_epoch >= ?
  `).get(e).cnt,c=n.file_reuse??0,d=n.content_cited??0,u=n.no_overlap??0,p=Math.max(0,o-a),g=c+d;return{file_reuse:c,content_cited:d,no_overlap:u,not_yet_computed:p,implicitUseRate:D(g,a)}}function Wt(t,e,s=30){let n=e.filter(b=>b.source==="semantic_prompt"),r=e.filter(b=>b.source==="semantic_summary"),o=e.filter(b=>b.source==="file_context"),i=e.filter(b=>b.status==="injected").length,a=e.filter(b=>b.status==="skipped").length,c=i+a,d=e.filter(b=>b.systemVerdict==="likely_helped").length,u=Te(t,s),p=Be(t,s),g=e.filter(b=>b.userFeedback==="helpful").length,f=e.filter(b=>b.userFeedback==="not_helpful").length,R=Date.now()-s*24*60*60*1e3,A=Gt(t,s),E=An(t,R),T=Oe(e,p.total,{scope:"global",key:"global"},A),y=[...new Set(e.map(b=>b.project).filter(b=>typeof b=="string"&&b.length>0))].sort((b,X)=>b.localeCompare(X)),O=Object.fromEntries(y.map(b=>{let X=e.filter(de=>de.project===b),xe=Be(t,s,{project:b}),rt=Gt(t,s,b);return[b,On(b,X,xe.total,rt)]})),h=y.flatMap(b=>["semantic_prompt","semantic_summary","file_context"].map(X=>{let xe=e.filter(de=>de.project===b&&de.source===X);return[`${b}::${X}`,Rn(b,X,xe,null)]})),C={semantic_prompt:Ve("semantic_prompt",n,null),semantic_summary:Ve("semantic_summary",r,null),file_context:Ve("file_context",o,null)},M=Date.now()-3600*1e3,I=e.filter(b=>b.createdAtEpoch>=M),P=I.filter(b=>b.status==="injected").length,_=I.filter(b=>b.status==="skipped").length,S=P+_,J=I.filter(b=>b.systemVerdict==="likely_helped").length,we={sinceEpoch:M,totalDecisions:I.length,injectRate:D(P,S),likelyHelpedRate:D(J,S),injected:P,actionable:S},zs={windowDays:s,totalDecisions:e.length,injected:i,injectRate:D(i,c),likelyHelped:d,likelyHelpedRate:D(d,c),recentTrend:we,userConfirmedHelpfulRate:D(g,g+f),estimatedInjectedTokens:T.estimatedInjectedTokens,helpfulRecallsPer1kInjectedTokens:T.helpfulRecallsPer1kInjectedTokens,injectedTokensPerLikelyHelpedRecall:T.injectedTokensPerLikelyHelpedRecall,taxonomyCorrectionRate:T.taxonomyCorrectionRate,helped:d,checkedNoHelp:Math.max(i-d,0),helpRate:D(d,i),feedback:u,helpful:u.helpful,notHelpful:u.notHelpful,bySource:u.bySource,sourceStats:C,projectStats:O,projectSourceStats:Object.fromEntries(h),availableProjects:y,taxonomyCorrections:p,shadowRanking:Bt(n),recommendation:T.recommendation,implicitUseRate:E.implicitUseRate,implicitUseCounts:{file_reuse:E.file_reuse,content_cited:E.content_cited,no_overlap:E.no_overlap,not_yet_computed:E.not_yet_computed}};return l.debug("DB",`memory-assist-dashboard: built for ${s}d window using ${e.length} decisions across ${y.length} projects`),zs}function w(t,e){return l.debug(`[MemoryAssistJudge] decision=${t.id??"new"} source=${t.source} status=${t.status} verdict=${e.verdict} confidence=${e.confidence.toFixed(2)}`),e}function se(){return{matchedTracePaths:[],usedOutcomes:[],ignoredOutcomes:[]}}function k(t){return t?t.replace(/\\/g,"/").trim().toLowerCase():null}function Nn(t){let e=new Set;for(let s of t.traceItems??[]){let n=k(s.filePath);n&&e.add(n);for(let r of s.relatedFilePaths??[]){let o=k(r);o&&e.add(o)}}return e}function In(t){let e=new Set;for(let s of t){let n=k(s.filePath);n&&e.add(n);for(let r of s.relatedFilePaths??[]){let o=k(r);o&&e.add(o)}}return e}function K(t,e){let s=new Set,n=k(t.filePath);n&&e.has(n)&&s.add(n);for(let r of t.relatedFilePaths??[]){let o=k(r);o&&e.has(o)&&s.add(o)}return[...s]}function vn(t,e,s){let n=new Set(e.generatedObservationIds??[]),r=new Set(s),o=new Set;for(let i of t.traceItems??[]){if(n.has(i.observationId)){o.add(i.observationId);continue}let a=k(i.filePath);if(a&&r.has(a)){o.add(i.observationId);continue}(i.relatedFilePaths??[]).some(d=>{let u=k(d);return u!=null&&r.has(u)})&&o.add(i.observationId)}return[...o]}function Cn(t,e){let s=new Set((e.concepts??[]).map(r=>r.trim().toLowerCase()).filter(Boolean));if(s.size===0)return 0;let n=new Set;for(let r of t.traceItems??[])for(let o of r.concepts??[]){let i=o.trim().toLowerCase();i&&s.has(i)&&n.add(i)}return n.size}function ne(t,e){return(t.generatedObservationIds??[]).some(s=>e.has(s))}function qt(t){return new Set(t)}function Kt(t,e){return e.size===0?!1:K(t,e).some(s=>e.has(s))}function W(t,e,s,n){let r=qt(s);return r.size===0?!1:e.some(o=>o===t||!n.includes(o.action)||(o.timestamp??0)>(t.timestamp??0)?!1:Kt(o,r))}function Jt(t,e,s,n){let r=qt(s);return r.size===0?!1:e.some(o=>o===t||!n.includes(o.action)||(o.timestamp??0)<(t.timestamp??0)?!1:Kt(o,r))}function Mn(t,e,s){return t.action==="browser"?"browser_follow_up":t.action==="command"?W(t,e,s,["edit","write"])?"terminal_follow_up":"other_follow_up":t.action==="read"?Jt(t,e,s,["edit","write"])?"follow_up_read":"other_follow_up":(t.action==="edit"||t.action==="write")&&W(t,e,s,["read"])?"follow_up_edit":"other_follow_up"}function Vt(t,e,s,n,r,o){let i=vn(t,e,n),a=Cn(t,e),c=Mn(e,s,n),d=(e.generatedObservationIds?.length??0)>0?"exact_observation_link":c==="follow_up_edit"||c==="terminal_follow_up"?"sequence_only":e.action==="browser"?"browser_only":n.length>0?"file_overlap":"no_overlap",u=d==="exact_observation_link"?o?"primary":"supporting":d==="sequence_only"?o?"supporting":"context":d==="file_overlap"&&o?"supporting":"context";return{outcomeId:e.id,pendingMessageId:e.pendingMessageId??null,action:e.action,toolName:e.toolName,filePath:e.filePath??null,timestamp:e.timestamp,matchedPaths:n,matchedTraceObservationIds:i,generatedObservationIds:e.generatedObservationIds??[],conceptOverlapCount:a,sequenceRole:c,signalSource:d,evidenceStrength:u,reason:r}}function Yt(t,e){return e.includes(t.action)}function B(t,e,s,n,r,o){let i=[],a=[];for(let c of e){let d=K(c,s);if(n(c,d)){i.push(Vt(t,c,e,d,r(c,d),!0));continue}a.push(Vt(t,c,e,d,o(c,d),!1))}return{matchedTracePaths:[...s],usedOutcomes:i,ignoredOutcomes:a}}function zt(t,e,s=t.userFeedback){if(s==="helpful")return w(t,{verdict:"likely_helped",confidence:.98,reasons:["User marked this memory assist as helpful."],evidence:se()});if(s==="not_helpful")return w(t,{verdict:"likely_not_helped",confidence:.98,reasons:["User marked this memory assist as not helpful."],evidence:se()});if(t.status!=="injected")return w(t,{verdict:"unclear",confidence:.4,reasons:["No memory was injected, so there is no direct adoption signal to judge."],evidence:se()});if(e.length===0)return w(t,{verdict:"unclear",confidence:.35,reasons:["No follow-up tool actions were recorded after this injection."],evidence:se()});if((t.traceItems?.length??0)===0&&(t.selectedCount??0)>0)return w(t,{verdict:"unclear",confidence:.3,reasons:["Summary injections have no file-path signals; adoption cannot be measured without explicit feedback."],evidence:se()});let r=t.promptNumber??null,o=new Set((t.traceItems??[]).map(_=>k(_.filePath)).filter(_=>!!_)),i=e.filter(_=>{if(_.action!=="read"||r==null||_.promptNumber==null||_.promptNumber!==r)return!0;let S=k(_.filePath);return S?!o.has(S):!0}),a=Nn(t),c=In(e),d=[...a].filter(_=>c.has(_)),u=d.length,p=new Set(d),g=e.filter(_=>K(_,p).length>0),f=g.filter(_=>_.action==="edit"||_.action==="write").length,R=i.filter(_=>_.action==="read"&&K(_,p).length>0).length,A=e.filter(_=>_.action==="browser").length,E=g.filter(_=>(_.generatedObservationIds?.length??0)>0).length,T=new Set((t.traceItems??[]).map(_=>_.observationId)),y=e.filter(_=>ne(_,T)),O=y.length,h=y.filter(_=>_.action==="edit"||_.action==="write").length,C=y.filter(_=>_.action==="read").length,M=g.filter(_=>(_.action==="edit"||_.action==="write")&&W(_,g,K(_,p),["read"])).length,I=g.filter(_=>_.action==="command"&&W(_,g,K(_,p),["edit","write"])).length;return h>0?w(t,{verdict:"likely_helped",confidence:M>0?.96:.9,reasons:[`${h} follow-up edit/write action${h===1?"":"s"} generated observations that were reused directly in the trace.`,...M>0?[`${M} of those edit/write action${M===1?"":"s"} followed a prior read on the same file.`]:[],...I>0?[`${I} same-target command follow-up${I===1?"":"s"} landed after an edit/write action.`]:[],"This is stronger evidence than plain file overlap because the exact generated observation linked back into the final trace."],evidence:B(t,e,p,(_,S)=>ne(_,T)&&Yt(_,["edit","write"]),(_,S)=>{let J=W(_,e,S,["read"]);return`Primary evidence: this ${_.action==="write"?"write":"edit"} generated observation content that was reused directly in the trace${J?", and it followed a same-target read":""}.`},(_,S)=>ne(_,T)?"Ignored by verdict because only edit/write actions count as adoption in this branch.":S.length>0?"Ignored by verdict because exact trace reuse outranked plain file overlap in this branch.":"Ignored by verdict because it did not generate trace-reused observations or overlap the injected memory paths.")}):C>0?w(t,{verdict:"likely_helped",confidence:.78,reasons:[`${C} follow-up read action${C===1?"":"s"} generated observations that were reused directly in the trace.`,"This is stronger evidence than plain file overlap, but weaker than seeing the same target edited afterward."],evidence:B(t,e,p,_=>ne(_,T)&&_.action==="read",()=>"Primary evidence: this read generated observation content that was reused directly in the trace.",(_,S)=>ne(_,T)?"Ignored by verdict because only read-based exact trace reuse counted in this branch.":S.length>0?"Ignored by verdict because exact trace reuse outranked plain file overlap in this branch.":"Ignored by verdict because it did not generate trace-reused observations or overlap the injected memory paths.")}):u>0&&f>0?w(t,{verdict:"likely_helped",confidence:O>0?.94:I>0?.91:E>0||M>0?.9:.88,reasons:[`Injected memory overlapped with ${u} file path${u===1?"":"s"} touched afterward.`,`${f} follow-up edit/write action${f===1?"":"s"} used those same files.`,...M>0?[`${M} of those edit/write action${M===1?"":"s"} followed a prior read on the same file.`]:[],...E>0?[`${E} matching follow-up action${E===1?"":"s"} produced exact linked observation${E===1?"":"s"}.`]:[],...O>0?[`${O} follow-up action${O===1?"":"s"} generated observations that were reused directly in the trace.`]:[],...I>0?[`${I} same-target command follow-up${I===1?"":"s"} landed after an edit/write action.`]:[]],evidence:B(t,e,p,(_,S)=>S.length>0&&Yt(_,["edit","write"]),(_,S)=>{let J=W(_,e,S,["read"]),we=Jt(_,e,S,["command"]);return`${(_.generatedObservationIds?.length??0)>0?"Primary":"Supporting"} evidence: it ${_.action==="write"?"wrote":"edited"} ${S.length===1?"the matched file":"matched files"}${(_.generatedObservationIds?.length??0)>0?` and generated ${_.generatedObservationIds.length} exact linked observation${_.generatedObservationIds.length===1?"":"s"}`:""}${J?", after a same-target read":""}${we?", with a same-target command follow-up afterward":""}.`},(_,S)=>S.length>0?`Ignored by verdict because only edit/write overlap counted here${(_.generatedObservationIds?.length??0)>0?", even though this tool action generated exact linked observations":""}.`:`Ignored by verdict because it did not overlap with the injected memory paths${(_.generatedObservationIds?.length??0)>0?", even though it generated exact linked observations":""}.`)}):I>0?w(t,{verdict:"likely_helped",confidence:.83,reasons:[`${I} same-target command follow-up${I===1?"":"s"} landed after an edit/write action on the matched files.`,"That is weaker than direct trace reuse, but stronger than plain overlap because the command followed work on the same target."],evidence:B(t,e,p,(_,S)=>_.action==="command"&&S.length>0&&W(_,e,S,["edit","write"]),(_,S)=>`Supporting evidence: this command followed an edit/write on ${S.length===1?"the matched file":"matched files"} and stayed on the same target set.`,(_,S)=>S.length>0?"Ignored by verdict because only same-target command follow-ups counted in this branch.":"Ignored by verdict because it did not stay on the injected memory target set.")}):u>0&&R>0&&(R>=2||E>0||O>0)?w(t,{verdict:"likely_helped",confidence:O>0?.84:E>0?.78:.74,reasons:[`Injected memory overlapped with ${u} file path${u===1?"":"s"} revisited afterward.`,`${R} follow-up read action${R===1?"":"s"} revisited the same files.`,...E>0?[`${E} matching follow-up action${E===1?"":"s"} produced exact linked observation${E===1?"":"s"}.`]:[],...O>0?[`${O} follow-up action${O===1?"":"s"} generated observations that were reused directly in the trace.`]:[]],evidence:B(t,e,p,(_,S)=>S.length>0&&_.action==="read",(_,S)=>`${(_.generatedObservationIds?.length??0)>0?"Primary":"Supporting"} evidence: it reread ${S.length===1?"the matched file":"matched files"}${(_.generatedObservationIds?.length??0)>0?` and generated ${_.generatedObservationIds.length} exact linked observation${_.generatedObservationIds.length===1?"":"s"}`:""}.`,(_,S)=>S.length>0?`Ignored by verdict because only read overlap counted in this branch${(_.generatedObservationIds?.length??0)>0?", even though this tool action generated exact linked observations":""}.`:`Ignored by verdict because it did not overlap with the injected memory paths${(_.generatedObservationIds?.length??0)>0?", even though it generated exact linked observations":""}.`)}):A>0?w(t,{verdict:"unclear",confidence:.46,reasons:["The follow-up signal was mostly browser/UI activity, which is weaker evidence than file overlap."],evidence:B(t,e,p,_=>_.action==="browser",()=>"Context-only evidence: browser/UI follow-up was the only available signal.",(_,S)=>S.length>0?"Ignored by verdict because browser/UI activity took precedence in this branch.":"Ignored by verdict because it did not overlap with the injected memory paths.")}):w(t,{verdict:"likely_not_helped",confidence:.62,reasons:["The injection was not followed by related file overlap or a concrete follow-up action."],evidence:B(t,e,p,()=>!1,()=>"Unused.",(_,S)=>S.length>0?"Ignored by verdict because there was overlap, but no qualifying follow-up action.":"Ignored by verdict because it did not overlap with the injected memory paths.")})}function Re(t){return t?t.replace(/\\/g,"/").trim().toLowerCase():null}function Dn(t){let e=new Set,s=Re(t.filePath);s&&e.add(s);for(let n of t.relatedFilePaths??[]){let r=Re(n);r&&e.add(r)}return e}function Ln(t){let e=new Set;for(let s of t.traceItems??[]){let n=Re(s.filePath);n&&e.add(n);for(let r of s.relatedFilePaths??[]){let o=Re(r);o&&e.add(o)}}return e}function qe(t){let e=t[0];for(let s=1;s<t.length;s++)t[s].createdAtEpoch>e.createdAtEpoch&&(e=t[s]);return e}function wn(t,e){let s=null,n=1/0;for(let r of t)if(r.createdAtEpoch<=e){let o=e-r.createdAtEpoch;o<n&&(s=r,n=o)}return s??qe(t)}function xn(t,e){return{customTitle:t,platformSource:e?Y(e):void 0}}var Ae=class{db;constructor(e=dt){e!==":memory:"&&lt(x),this.db=new Qt.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureObservationFeedbackTable(),this.ensureMemoryAssistTables(),this.createObservationsFTSIndex(),this.addObservationDecisionDNAFields(),this.addCaptureSnapshotTables(),this.addObservationContextOriginFields(),this.addMcpInvocationsTable(),this.addMemoryImplicitSignalsTable(),this.addLlmRawTypeColumn()}createObservationsFTSIndex(){let e=this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='observations_fts'").all().length>0;try{e||(this.db.run(`
    ...
    `).get(e);if(u){if(s&&this.db.prepare(`
        ...
        `).run(c.customTitle,e),c.platformSource){let g=u.platform_source?.trim()?Y(u.platform_source):void 0;if(!g)this.db.prepare(`
    ...
    `).run(e,s,n,r.toISOString(),o).lastInsertRowid}getUserPrompt(e,s){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,n,r,o=0,i,a,c){let d=i??Date.now(),u=new Date(d).toISOString(),p=_e(e,n.title,n.narrative),g=me(this.db,p,d);if(g)return{id:g.id,createdAtEpoch:g.created_at_epoch};let R=this.db.prepare(`
    ...
    `).run(e,s,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,r||null,o,c,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,s,n,r,o,i=0,a,c,d){let u=a??Date.now(),p=new Date(u).toISOString();return this.db.transaction(()=>{let f=[],R=this.db.prepare(`
      ...
      `),A=d??Ee(e,null,o??null);for(let T of n){let y=_e(e,T.title,T.narrative),O=me(this.db,y,u);if(O){f.push(O.id);continue}let h=R.run(e,s,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,y,p,u,c||null,T.why??null,T.alternatives_rejected??null,T.related_observation_ids&&T.related_observation_ids.length>0?JSON.stringify(T.related_observation_ids):null),C=Number(h.lastInsertRowid);f.push(C),pe(this.db,C,A,ge(T),u)}let E=null;if(r){let y=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         ...
         generated_by_model, why, alternatives_rejected, related_observation_ids)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `),T=p??Ee(e,null,a??null);for(let h of n){let C=_e(e,h.title,h.narrative),M=me(this.db,C,g);if(M){A.push(M.id);continue}let I=E.run(e,s,h.type,h.title,h.subtitle,JSON.stringify(h.facts),h.narrative,JSON.stringify(h.concepts),JSON.stringify(h.files_read),JSON.stringify(h.files_modified),a||null,c,C,f,g,u||null,h.why??null,h.alternatives_rejected??null,h.related_observation_ids&&h.related_observation_ids.length>0?JSON.stringify(h.related_observation_ids):null),P=Number(I.lastInsertRowid);A.push(P),pe(this.db,P,T,ge(h),g)}let y;if(r){let C=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
