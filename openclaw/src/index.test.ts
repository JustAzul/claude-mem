...
describe("Observation I/O event handlers", () => {
  ...
  it("session_start tracks session without initing the worker", async () => {
    ...
    assert.ok(!initRequest, "session_start should defer init until before_agent_start");
  ...
  it("session_start does not call init on worker", async () => {
    ...
    assert.equal(initRequests.length, 0, "session_start should not init the worker");
  });

  it("after_compaction preserves tracking without re-init", async () => {
    const { api, logs, fireEvent } = createMockApi({ workerPort });
    claudeMemPlugin(api);
    ...
    const initRequests = receivedRequests.filter((r) => r.url === "/api/sessions/init");
    assert.equal(initRequests.length, 0, "after_compaction should preserve tracking without re-init");
  ...
  it("tool_result_persist sends observation to worker", async () => {
    ...
    await fireEvent("before_agent_start", { prompt: "inspect src/index.ts" }, { sessionKey: "test-agent" });
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    }, { sessionKey: "test-agent", workspaceDir: "/tmp/project" });
  ...
  it("tool_result_persist truncates long responses", async () => {
    ...
    await fireEvent("before_agent_start", { prompt: "run bash ls" }, { sessionKey: "truncate-test" });
    await new Promise((resolve) => setTimeout(resolve, 100));
    await fireEvent("tool_result_persist", {
      toolName: "Bash",
      params: { command: "ls" },
    ...
    }, { sessionKey: "truncate-test", workspaceDir: "/tmp/project" });
  ...
  it("reuses same contentSessionId for same sessionKey", async () => {
    ...
    await fireEvent("before_agent_start", { prompt: "reuse me" }, { sessionKey: "reuse-test" });
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    }, { sessionKey: "reuse-test", workspaceDir: "/tmp/project" });
